...
const { pool } = require("../db-connect/postgresDB");
  ...
  if (Object.keys(req.query).length === 0) {
    ...
    try {
      ...
      const error = new HttpError(
        "Something went wrong, could not find extra jobs. ",
        500
      );
      return next(error);
    }
    ...
    if (!allExtraJobs.rows) {
      const error = new HttpError("Could not find any extra job", 404);
      return next(error);
    }
    ...
    try {
      allExtraJobs = await pool.query(
        "SELECT * FROM extra_jobs \
                WHERE city LIKE $1 AND category LIKE $2",
        [city, category]
      );
    } catch (err) {
      console.log(err);
...
const getExtraJobByUserId = async (req, res, next) => {
  ...
  let extraJobs;
  try {
    extraJobs = await pool.query(
      "SELECT DISTINCT extra_jobs.title, extra_jobs.description, extra_jobs.company_id, extra_jobs.job_id, extra_jobs.salary, extra_jobs.address \
        FROM extra_jobs \
        INNER JOIN extra_job_applications ON extra_job_applications.user_id = $1",
      [userId]
    );
  } catch (err) {
    const error = new HttpError(
      "Something went wrong, could not find a extra job. ",
      500
    );
    return next(error);
  }
...
const createExtraJob = async (req, res, next) => {
  ...
  try {
    existingComapny = await pool.query(
  ...
  try {
    createdExtraJob = await pool.query(
      "INSERT INTO extra_jobs \
        (company_id, title, description, category, address \
          , city, coordinate_long, coordinate_let, locality, country, country_code \
          , date, created_date, salary_range) \
    ...
    console.log(err);
    const error = new HttpError(
      "Creating extra job failed, please try again.",
      500
    );
    return next(error);
  }
...
const updateExtraJob = async (req, res, next) => {
  ...
  let updatedExtraJob;
  try {
    updatedExtraJob = await pool.query(
      "SELECT * \
        FROM extra_jobs \
        WHERE id = $1",
      [extraJobId]
    ...
    console.log(err);
    const error = new HttpError(
      "Something went wrong, could not find the job. ",
      500
    );
    return next(error);
  }
  ...
  let coordinate_let;
  if (title == null) {
    title = updatedExtraJob.rows[0].title;
  }
  if (description == null) {
    description = updatedExtraJob.rows[0].description;
  }
  if (address == null) {
    address = updatedExtraJob.rows[0].address;
  }
  if (salary == null) {
    salary = updatedExtraJob.rows[0].salary;
  }
  if (category == null) {
    category = updatedExtraJob.rows[0].category;
  }
  if (date == null) {
    date = updatedExtraJob.rows[0].date;
  }
  if (city == null) {
    city = updatedExtraJob.rows[0].city;
  }
  ...
  try {
    newExtraJob = await pool.query(
      "UPDATE extra_jobs \
        SET title = $1, description = $2, address = $3, city = $4,\
        salary = $5, coordinate_long = $6, coordinate_let = $7, changed_date = $8 \
        WHERE id = $9",
      [
        title,
        description,
    ...
    console.log(err);
    const error = new HttpError("Updating Job failed, please try again.", 500);
    console.log(err);
    return next(error);
  }
...
const deleteExtraJob = async (req, res, next) => {
  ...
  try {
    existingExtraJob = await pool.query(
      "SELECT * FROM extra_jobs WHERE id = $1",
      [extraJobId]
    );
  } catch (err) {
    console.log(err);
  ...
  try {
    deletedExtraJob = await pool.query(
